...
def lm(varObjCont, datos, var_cont, var_categ, var_interac=[]):
    ...
    # Construir matriz de diseño homogénea (todo numérico) + constante
    X = sm.add_constant(datos, has_constant="add").astype(float)

    # Asegurar y como numérica (y alineada por índice si aplica)
    if isinstance(varObjCont, pd.Series):
        y = pd.to_numeric(varObjCont, errors="raise").astype(float)
        y = y.loc[X.index]
    else:
        y = pd.to_numeric(pd.Series(varObjCont, index=X.index), errors="raise").astype(float)
